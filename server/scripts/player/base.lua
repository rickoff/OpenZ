...
function BasePlayer:EndCharGen()
    ...
    if spawnUsed ~= nil and spawnUsed.cellDescription ~= nil then
		...
		for _, item in ipairs(itemTab) do
			self.data.equipment[item.index] = {
				refId = item.refId,
				count = item.count,
				charge = item.charge,
				enchantmentCharge = item.enchantmentCharge
			}
			tes3mp.EquipItem(self.pid, item.index, item.refId, item.count, item.charge, item.enchantmentCharge)
		end
...
function BasePlayer:SaveAttributes(playerPacket)
    ...
    for attributeName in pairs(self.data.attributes) do
        ...
        elseif (attribute.base + attribute.modifier) > maxAttributeValue and not config.ignoreModifierWithMaxSkill then
